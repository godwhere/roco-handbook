...
| 数据规模 | 621 个具体精灵形态、466 个图鉴条目、824 个技能、311 份技能学习表、273 组进化关系 |
...
| 平台视觉 | iOS 26 使用原生 Liquid Glass 导航，旧版 iOS 使用系统材质降级；Android 使用 M3 Expressive 导航与色调层级 |
| 数据更新 | 设置页主动检查，使用 GitHub Releases 分发由 Ed25519 签名清单认证的完整图鉴包 |
| 自动化验证 | 105 项 Python 测试、118 项 Flutter 测试、静态检查、格式检查和 GitHub Actions 离线持续集成 |
...
- 默认双列卡片放大展示完整立绘，也可在设置中切换回紧凑横向列表；两种布局都保留形态副标题、`NO.<编号>`、系别图标和详情跳转。
...
- iOS 的顶部和底部导航由 UIKit 原生组件承载；在 iOS 26 上使用系统 Liquid Glass，在旧系统上自动降级为系统材质。
- Android 保留 Material 3 组件体系，通过更明确的圆角、色调层级、选中指示器和反馈呈现 M3 Expressive。
- 两端共享同一套离线数据和页面结构，但不强行复制对方的视觉语言；玻璃只用于 iOS 导航层，图鉴卡片、技能说明和长文区域保持清晰的实体背景。
...
- 当前 NRC 注册表锁定 1,911 个精灵、图鉴和技能身份，并检查缺失、复用和未审核变更。
- 新版来源直接提供图鉴编号、默认展示顺序和归属赛季，不再通过名称、后缀或列表顺序推断。
...
- 595 张去重原始精灵立绘覆盖全部 621 个具体形态。
- 191 张去重异色立绘覆盖 193 个可切换引用。
- 773 个技能或特性图标、35 个属性与分类图标随 App 打包。
- 图鉴视觉资源清单包含 1,594 个文件、1,673 个引用和 124,766,985 字节，并记录来源与本地哈希。
...
- 阶段 7 已实现完整图鉴包构建、签名、GitHub Releases 传输、前台下载和安全安装；当前已生成真实的 NRC Catalog 数据版本 2，但尚未制作、签名或发布对应的远端更新包。
- 阶段 8 已完成正式 App 身份、中英文界面、离线图片与字体、图鉴与详情页重构、筛选、异色切换、七个工具入口，以及 iOS 原生 Liquid Glass／Android M3 Expressive 平台导航，并已将新版 NRC Wiki 数据与图片作为发布前基线；应用商店截图需要在最终视觉确定后重新生成。
- Android 模拟器已验证飞行模式首次启动、个人数据持久化、旧指针迁移、恢复流程和当前 M3 Expressive 首页呈现。
- iOS 模拟器已验证构建、首次启动、数据库创建与复用、旧指针迁移、当前主要界面流程和 iOS 26 原生 Liquid Glass 导航呈现。

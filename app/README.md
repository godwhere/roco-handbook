...
The production App does not request BWIKI, execute Lua, require an account, or download data on first launch. Settings is the only trigger for its fixed GitHub Releases Catalog source; no startup, automatic, or background update exists. Canonical upstream names and descriptions retain their source language. The interface supports `en-US` and source-grounded `zh-CN`, with English fallback for every other locale. The English UI name remains **Roco World Handbook**; the Chinese UI and both platform home-screen labels use the shorter product name fixed by ADR-0015.
...
Navigation intentionally follows each platform. iOS uses a bounded UIKit bridge for native `UINavigationBar` and `UITabBar` rendering: iOS 26 receives system Liquid Glass, while iOS 15 through 25 receive the native system-material fallback. Android stays in Flutter Material 3 and uses expressive rounded navigation shapes, tonal elevation, and selected-state emphasis. Glass never extends into Catalog cards, sheets, or long-form content.
...
The creature catalog lists concrete forms with Sort, Filters, and Search on one row and uses full illustrations instead of head images. Its default responsive grid presents two large cards per phone row and three on wider surfaces; Settings can restore the compact horizontal list. The typed choice is retained in the existing device-local `user.db.settings` store and survives Catalog replacement. The vertically grouped filter sheet combines shiny availability, stage, main/regional/lord form, S1 through S4 owning season, and type. Search uses a compact rounded outline without a floating label. Both card layouts keep the source name, muted non-default form label, `NO.<dex_no>`, accessible type icons, season-colored shiny-capable names, and detail navigation while reserving favorite actions for the detail header.
...
The skill destination is a learnable-skill handbook with a one-row Skill handbook, Skill filters, and normal text-input Skill query toolbar. Its filter sheet combines four skill types, 20 documented read-only source-text labels, and 18 stored elements. Result cards show the frozen icon, name with element icon, category, energy, power, and source description; feature relations remain accessible from creature details, and skill favorites live in the skill detail header. The frozen NRC image manifest under `assets/wiki/v3/` covers every active original creature illustration, every shiny-capable illustration reference, and every active skill reference.

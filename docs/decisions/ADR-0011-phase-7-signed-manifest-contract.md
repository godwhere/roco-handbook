...
- A caller-provided validation context must prove that the App and protocol are compatible, the Catalog schema matches, the signing key covers the release sequence, and the package is within the configured size limit. Installation requires both target data version and release sequence to be newer. Discovery may classify a fully validated manifest as current only when both values are already accepted; mixed stale/new combinations remain errors.
...
Following explicit custody authorization, `catalog-prod-2026-01` is the first production Catalog key. Its public key is the sole entry in `app/assets/catalog/catalog_trust_store.json`, beginning at release sequence 1 with no current upper bound. The unencrypted private JSON is outside the repository under `~/Desktop/RocoWorld-Catalog-Signing/`, inside an owner-only directory with an owner-only file. A later read-only check confirmed that macOS iCloud Desktop synchronization is enabled and that this file is visible through the iCloud Desktop path. The repository owner explicitly accepted that synchronized custody model on 2026-09-10. An encrypted offline backup remains a separate future decision.
...
The repository has a real publisher-authentication boundary, a public production trust root, an accepted external private-key custody location, and executable negative conformance tests. ADR-0013 subsequently selected GitHub Releases and connected this verifier to a bounded foreground discovery and download flow. A real production payload and Release remain absent until a newer validated Catalog exists.
